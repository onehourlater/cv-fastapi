...
from .schema import UserBase, UserBaseAuth
...
async def signup(
    auth_data: UserBaseAuth, auth_manager=Depends(get_auth_manager)
) -> UserBase:
    print()
...
async def signin(
    ...
    refresh_token, refresh_token_expires = create_JWT_token(
        data={auth_settings.JWT_USERNAME_KEY: user.username}, is_refresh=True
    )
    return AuthTokens(
        token_type='bearer',
        access_token=access_token,
        access_expires_seconds=access_token_expires,
        refresh_token=refresh_token,
        refresh_expires_seconds=refresh_token_expires,
    )
